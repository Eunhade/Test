...
document.getElementById("register").onclick = async () => {
  ...
  console.log('Register button clicked', { username, passwordLength: password.length });
...
document.getElementById("login").onclick = async () => {
  ...
  console.log('Login button clicked', { username, passwordLength: password.length });
...
function initSocket() {
  ...
  socket.on("score_update", (data) => {
    document.getElementById("scoreP1").textContent = data.p1;
    document.getElementById("scoreP2").textContent = data.p2;
  });
...
function handleGameOver(data) {
  ...
  showStatus(message, p1Score > p2Score ? "success" : "info");
